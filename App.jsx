...
async function generatePDF(proyectos, añosInicio, enabled, totals, paramsMap, horizon, precios, chartsRef) {
  ...
  const addPageHeader = () => {
    ...
    doc.text('Impacto Económico — Cartera Minera Argentina', 8, 12)
    doc.setFontSize(7.5); doc.setFont('helvetica','normal'); doc.setTextColor(148,163,184)
    doc.text(`${TODAY_LABEL}  ·  Horizonte ${horizon}  ·  Simulación`, W-5, 12, { align:'right' })
  ...
  // Header de tabla: fondo azul claro, texto blanco
  const tblHead = (cols, widths, aligns, y, bgR=2, bgG=132, bgB=199) => {
    ...
    cols.forEach((h,i)=>{
      const a = aligns[i]
      if (a==='right') doc.text(h, x+widths[i]-2, y, {align:'right'})
  ...
  // Fila de datos con línea inferior sutil
  const tblRow = (vals, widths, aligns, y, idx, textColors=null) => {
    if (idx%2===0) { doc.setFillColor(248,250,252); doc.rect(5,y-4,W,6.5,'F') }
    else           { doc.setFillColor(255,255,255); doc.rect(5,y-4,W,6.5,'F') }
  ...
  const newPage = () => { doc.addPage(); addPageHeader(); return 24 }
  ...
  let y = sectionTitle('Parámetros de simulación por proyecto', 71)
  ...
  proyectos.forEach((p,idx) => {
    ...
    const on  = enabled[p.nombre]!==false
    ...
    if (!on) {
      ...
      doc.line(7,y-1,W,y-1); doc.setLineDashPattern([],0)
    }
    y+=6.5
  })
  ...
  y = newPage()
  y = sectionTitle('Flujos anuales agregados — proyectos activos (USD millones)', y)

  const fCols  = ['Año','Inv.$M','Cob.Inv','Lit.Inv','Oro.Inv','Emp.C','Emp.O','Emp.Tot','Valor$M','Cob.Val','Lit.Val','Oro.Val','InsNac$M','InsImp$M']
  const fW     = [14,18,16,16,16,16,14,18,18,16,16,16,18,18]
  const fAlign = ['left','right','right','right','right','right','right','right','right','right','right','right','right','right']
  ...
  totals.filter(t=>t.año>=2025&&t.año<=horizon&&(t.inv>0||t.valor>0||t.empTot>0))
    .forEach((t,idx)=>{
      ...
      const colC=[
        isNow?[220,38,38]:[30,41,59],
        [2,132,199],[2,132,199],[5,150,105],[217,119,6],
        [5,150,105],[5,150,105],[5,150,105],
        [217,119,6],[2,132,199],[5,150,105],[217,119,6],
        [124,58,237],[220,38,38]
      ]
      tblRow([
        String(t.año),
        t.inv>0?String(Math.round(t.inv/1e6)):'',
        t.g.cobre.inv>0?String(Math.round(t.g.cobre.inv/1e6)):'',
        t.g.litio.inv>0?String(Math.round(t.g.litio.inv/1e6)):'',
        t.g.oro_plata.inv>0?String(Math.round(t.g.oro_plata.inv/1e6)):'',
        t.empC>0?fmtK(t.empC):'', t.empO>0?fmtK(t.empO):'', t.empTot>0?fmtK(t.empTot):'',
        t.valor>0?String(Math.round(t.valor/1e6)):'',
        t.g.cobre.valor>0?String(Math.round(t.g.cobre.valor/1e6)):'',
        t.g.litio.valor>0?String(Math.round(t.g.litio.valor/1e6)):'',
        t.g.oro_plata.valor>0?String(Math.round(t.g.oro_plata.valor/1e6)):'',
        t.insNac>0?String(Math.round(t.insNac/1e6)):'',
        t.insImp>0?String(Math.round(t.insImp/1e6)):''
      ], fW, fAlign, y, idx, colC)
      doc.setFont('helvetica','normal')
      y+=6.5
    })
  ...
  // ── PÁGINA 3: Precios ───────────────────────────────
  y = newPage()
  y = sectionTitle('Precios de referencia por mineral — año a año', y)

  const prCols  = ['Año','Cobre (USD/t)','Litio (USD/t LCE)','Oro (USD/oz)','Plata (USD/oz)']
  const prW     = [16,65,70,55,55]
  const prAlign = ['left','right','right','right','right']
  const minC    = { cobre:[2,132,199], litio:[5,150,105], oro:[217,119,6], plata:[107,114,128] }
  ...
  for (let yr=2025; yr<=horizon; yr++) {
    ...
    if (isNow) { doc.setFillColor(254,242,242); doc.rect(5,y-4,W,6.5,'F') }
    doc.setFont('helvetica',isNow?'bold':'normal')
    const vals=['cobre','litio','oro','plata'].map(m=>{
    ...
    tblRow([String(yr),...vals], prW, prAlign, y, (yr-2025),
      [isNow?[220,38,38]:[30,41,59], ...['cobre','litio','oro','plata'].map(m=>minC[m])])
    doc.setFont('helvetica','normal')
    y+=6.5
  }
  ...
  if (y>185) { y=newPage() }
  doc.setFontSize(6.5); doc.setTextColor(148,163,184)
  doc.text('Fuentes: Cochilco (cobre 2025–2027) · Goldman Sachs dic 2025 (cobre LP) · Fastmarkets/Benchmark MI (litio) · World Gold Council (oro) · Silver Institute (plata).', 5, y+8)
  doc.text('Precios 2041–2075 son estimaciones de largo plazo de alta incertidumbre. Actualizables en PRECIOS_REFERENCIA de la sheet.', 5, y+13)

  // ── PÁGINA 4: Gráficos del dashboard ───────────────
  if (chartsRef?.current) {
    try {
      ...
      doc.addPage()
      addPageHeader()
      let yc = sectionTitle('Visualizaciones del dashboard', 24)

      // Si la imagen entra en una página
      if (imgH <= 175) {
        doc.addImage(imgData, 'PNG', 5, yc, imgW, imgH)
      } else {
        // Partir en dos mitades (superior / inferior)
        const halfH = imgH / 2
        doc.addImage(imgData, 'PNG', 5, yc, imgW, imgH, '', 'FAST', 0)
        if (yc + halfH > 196) {
  ...
  doc.save(`impacto-minero-${TODAY}.pdf`)
}
      ...
      <header className="bg-gray-900 text-white px-4 py-3 shadow">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={() => generatePDF(data.proyectos, añosInicio, enabled, totals, paramsMap, horizon, data.precios, chartsRef)}
